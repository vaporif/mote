...
use crate::provider::annotation_extractors::extract_string_eq;

use super::references_column;
use arrow::{
...
pub struct HistoryStringAnnotationsProvider {
    conn: Arc<Mutex<Connection>>,
    schema: SchemaRef,
}

impl fmt::Debug for HistoryStringAnnotationsProvider {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("HistoryStringAnnotationsProvider").finish()
    }
}

impl HistoryStringAnnotationsProvider {
    pub fn new(conn: Arc<Mutex<Connection>>) -> Self {
        Self {
            conn,
            schema: glint_primitives::exex_schema::history_string_annotations_schema(),
        }
    }
}

#[async_trait]
impl TableProvider for HistoryStringAnnotationsProvider {
    ...
    async fn scan(
        ...
        let batch = tokio::task::spawn_blocking(move || {
            let conn = conn.lock();
            query_history_string_annotations(
                &conn,
                range.0,
                range.1,
                ann_key.as_deref(),
...
fn query_history_string_annotations(
    conn: &Connection,
    from_block: u64,
    to_block: u64,
    ann_key: Option<&str>,
